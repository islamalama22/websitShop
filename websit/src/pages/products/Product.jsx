...
return (
  <>
      ...
      <Card sx={{ padding: 3 }}>
        <Grid sx={{ display:"flex", alignContent:"center", alignItems:"center" }} container spacing={3}>
          <Grid item size={{ xs:12, md:5 }}>
            <CardMedia component={'img'} image={product.image} sx={{ height:300, objectFit:'contain' }} />
          </Grid>
          ...
          <Grid sx={{ display:"flex", flexDirection:"column", gap:1, alignItems:'flex-start' }} item size={{ xs:12, md:7 }}>
            ...
            <Typography component={'h3'} variant='h3'>available quantity: {product.quantity}</Typography>
            <Button
    ...
    <Box component={'section'} sx={{ display:'flex', flexDirection:'column', padding:2, gap:2 }}>
      <Typography variant='h6'>Users Reviews</Typography>
      ...
      {canReviwe ? (
        <>                                          {/* ✅ Fragment opens */}
          ...
          {reviwes.map((reviwe) => (
            <Box
              key={reviwe.id}                       {/* ✅ key added */}
              sx={{ padding:1.5, border:1, borderColor:'rgb(186,186,181)', borderRadius:1 }}
            >
              ...
              <Box sx={{ display:"flex", justifyContent:'space-between' }}>
                ...
                <Rating value={Number(reviwe.rating)} readOnly />
              </Box>
            </Box>
          ))}
        </>                                        
        ...
        <Typography color='error'>
          You can't add a review unless you have received your order.
        </Typography>
      )}
